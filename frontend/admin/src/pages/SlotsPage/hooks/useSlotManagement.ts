...
export const useSlotManagement = (selectedWorkshopId: string | null) => {
  ...
  const createMultipleSlots = async (formData: SlotFormData): Promise<boolean> => {
    ...
    try {
      ...
      setFeedback({
        message: `Utworzono pomyślnie ${createdCount} ${termText}`,
        type: 'success'
      })
      return true
    } catch (error) {
  ...
  const createSlotsFromTemplate = async (template: any, date: Date): Promise<boolean> => {
    ...
    try {
      ...
      setFeedback({
        message: `Utworzono ${createdCount} terminów z szablonu "${template.name}"`,
        type: 'success'
      })
      return true
    } catch (error) {
  ...
  const bulkDeleteSlots = async (slotIds: string[]): Promise<boolean> => {
    ...
    try {
      ...
      setFeedback({
        message: `Usunięto ${slotIds.length} terminów`,
        type: 'success'
      })
      return true
    } catch (error) {
      setFeedback({
        message: 'Wystąpił błąd podczas usuwania terminów',
        type: 'error'
      })
      return false
    }
